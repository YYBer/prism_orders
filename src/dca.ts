...
export class HodlLadderDCA {
     ...
     */
    private getBaseTokens(): Record<string, string> {
        return {
            // Base mainnet tokens
     ...
     */
    private suggestTokenPairs(): void {
    ...
    public async validateConfigurationWithResult(): Promise<ValidationResult> {
        ...
        try {
            ...
            if (!ethers.utils.isAddress(this.config.fromToken) || !ethers.utils.isAddress(this.config.toToken)) {
                errors.push('Invalid token addresses provided');
                return { isValid: false, errors, warnings };
            }

            // Get token info
            const fromTokenContract = new ethers.Contract(this.config.fromToken, ERC20_ABI, this.provider);
            const toTokenContract = new ethers.Contract(this.config.toToken, ERC20_ABI, this.provider);
    ...
    private async createSingleOrderWithSDK(orderIndex: number, amountPerOrder: bigint): Promise<OrderData | null> {
        try {
            ...
            const limitOrder = new LimitOrder({
                ...
                maker: new OneInchAddress(await this.signer.getAddress()),
                salt: randBigInt(2n ** 256n - 1n), // Random salt
                receiver: new OneInchAddress(await this.signer.getAddress())
            }, makerTraits);
     ...
     */
    private calculateTargetPrice(currentPrice: number, orderIndex: number): number {
    ...
    private calculateTakingAmountBigInt(makingAmount: bigint, targetPrice: number): bigint {
        ...
        return ethers.utils.parseUnits(takingAmountWithSlippage.toFixed(this.config.toTokenDecimals), this.config.toTokenDecimals).toBigInt();
    }
    ...
    private async getCurrentPrice(): Promise<number> {
        try {
            ...
            console.error('❌ Failed to fetch price from 1inch API:', (error as any).response?.data || (error as Error).message);
     ...
     */
    public async getActiveOrdersFromAPI(): Promise<OneInchOrderInfo[]> {
        try {
            ...
            ordersResponse.forEach((order: any, index: number) => {
            ...
            const activeOrders = ordersResponse.filter((order: any) => {
     ...
     */
    public async monitorOrders(): Promise<void> {
     ...
     */
    public async submitOrdersToProtocol(): Promise<{success: boolean, orderHash?: string, error?: string}[]> {
        console.log('📤 Orders already submitted to 1inch protocol via SDK...');
        
        const results: {success: boolean, orderHash?: string, error?: string}[] = [];
    ...
    public async monitorAndExecute(): Promise<void> {
        ...
        const monitoringInterval = setInterval(async () => {
            try {
                ...
                console.error('❌ Error during monitoring:', (error as Error).message);
            }
     ...
     */
    private async checkOrderExecutionWithAPI(): Promise<void> {
        try {
            ...
            for (const orderInfo of activeOrders) {
                ...
                if (orderData && this.shouldExecuteOrder(orderData, currentPrice)) {
                    ...
                    if ((updatedOrderInfo as any).fillableBalance === '0') {
            ...
            console.error('❌ Error checking order execution:', (error as Error).message);
        }
    }
     ...
     */
    private shouldExecuteOrder(orderData: OrderData, currentPrice: number): boolean {
        switch (this.config.strategyType) {
            case 1: // Price Drop DCA
     ...
     */
    private getStrategyTypeName(): string {
        switch (this.config.strategyType) {
     ...
     */
    public async getOrderStatus(orderHash: string): Promise<OneInchOrderInfo | null> {
        try {
            ...
            return {
                ...
                orderInvalidReason: (result as any).orderInvalidReason,
                auctionStartDate: (result as any).auctionStartDate,
                auctionEndDate: (result as any).auctionEndDate,
                remainingMakerAmount: (result as any).remainingMakerAmount,
                makerBalance: (result as any).makerBalance,
                makerAllowance: (result as any).makerAllowance
            };
        } catch (error) {
            console.error(`❌ Failed to get order status for ${orderHash.slice(0, 10)}...:`, (error as Error).message);
            return null;
        }
    }
    ...
    public async syncOrdersWithAPI(): Promise<void> {
        try {
            ...
            console.error('❌ Error syncing orders with API:', (error as Error).message);
        }
    }
}
