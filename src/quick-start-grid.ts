...
import { VolatilityGridStrategy } from './grid';
...
// Pre-configured Volatility Grid Strategy Definitions
interface PreConfiguredGridStrategy {
  name: string;
  description: string;
  config: Omit<StrategyConfig, 'fromToken' | 'toToken'> & {
    baseTokenSymbol: keyof typeof BASE_TOKENS;
    quoteTokenSymbol: keyof typeof BASE_TOKENS;
    gridLevels: number;
    priceRange: number;
    profitTarget: number;
...
const GRID_STRATEGIES: Record<string, PreConfiguredGridStrategy> = {
  '1': {
    ...
    config: {
      ...
      totalAmount: '10',
      numberOfOrders: 10,
      strategyType: StrategyType.PRICE_DROP_DCA, // Used for configuration mapping
      gridLevels: 10,
      priceRange: 10, // ±10%
...
class VolatilityGridQuickStartApp {
  ...
  private async runInteractiveMode(): Promise<void> {
    ...
    const gridStrategy = new VolatilityGridStrategy(this.provider, this.signer);
    await gridStrategy.initialize();
    await gridStrategy.executeGridStrategy();
  }

  private async confirmStrategy(config: StrategyConfig, gridConfig: PreConfiguredGridStrategy['config']): Promise<boolean> {
    ...
    try {
      ...
      const baseBalanceFormatted = parseFloat(ethers.utils.formatUnits(baseBalance, config.fromToken.decimals));
      
      // Check quote token balance
      const quoteTokenContract = new ethers.Contract(
        config.toToken.address, 
        ['function balanceOf(address) view returns (uint256)'], 
        this.provider
      );
      const quoteBalance = await quoteTokenContract.balanceOf(walletAddress);
      const quoteBalanceFormatted = parseFloat(ethers.utils.formatUnits(quoteBalance, config.toToken.decimals));
  ...
  private async executeGridStrategy(config: StrategyConfig, gridConfig: PreConfiguredGridStrategy['config']): Promise<void> {
    try {
      ...
      console.error('❌ Volatility Grid strategy execution failed:', (error as Error).message);
  ...
  private showGridMonitoringInstructions(): void {
  ...
  private async startAdvancedGridMonitoring(gridStrategy: VolatilityGridStrategy): Promise<void> {
...
function showGridAnalytics(): void {
...
export { VolatilityGridQuickStartApp };